...
class User < ApplicationRecord
  ...
  def update_from_omniauth!(auth_data)
    ...
    oauth_scope = credentials["scope"].to_s.presence
    if oauth_token.present?
      self.github_oauth_token = oauth_token
      self.github_oauth_scope = oauth_scope
      self.github_oauth_connected_at = Time.current
    end

    save!
  end

  def github_oauth_connected?
    github_oauth_token.present?
  end

  def github_oauth_token_for_insights
    github_oauth_token.to_s.presence
  end
