  ...
  class RateLimiter
    ...
    def allowed?(user:, project:)
      ...
      if daily_count(user) >= DAILY_LIMIT
        return [false, "Daily Project Story Builder limit reached. Try again tomorrow."]
      end

      if cooldown_active?(user, project)
        return [false, "Please wait a few seconds before generating another story draft for this project."]
      end
    ...
    def daily_key(user)
      ...
      "project_story_builder:daily:#{user.id}:#{date}"
    end

    def cooldown_key(user, project)
      "project_story_builder:cooldown:#{user.id}:#{project.id}"
    end
