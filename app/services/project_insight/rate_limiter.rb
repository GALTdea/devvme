  ...
  class RateLimiter
    ...
    def allowed?(user:, project:)
      ...
      if daily_count(user) >= DAILY_LIMIT
        return [false, "Daily question limit reached for Project Insight"]
      end

      if cooldown_active?(user, project)
        return [false, "Please wait a few seconds before asking another question about this project"]
      end
    ...
    def daily_key(user)
      ...
      "project_insight:daily:#{user.id}:#{date}"
    end

    def cooldown_key(user, project)
      "project_insight:cooldown:#{user.id}:#{project.id}"
    end
