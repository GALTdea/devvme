class DropWaitingListEntries < ActiveRecord::Migration[8.0]
  def up
    # Drop foreign key first
    remove_foreign_key :waiting_list_entries, :users if foreign_key_exists?(:waiting_list_entries, :users)

    # Drop the table (indexes are automatically dropped with the table)
    drop_table :waiting_list_entries if table_exists?(:waiting_list_entries)
  end
